...
window.onload= function() {
	document.querySelector('.page-range').style.display= 'none';
}
...
var Pagination = function (el, options) {
	...
	var $container = $(el),
		...
		firstPageLabel = options.labels.first || '&laquo;',
		lastPageLabel = options.labels.last || '&raquo;',
	...
	getRenderedPageNumbers = function () {
		...
		$.each(pageNumbers, function (i, p) {
			result += disptext+'<li data-page="' + p + '" ' + (p === currentPage ? 'class="active"' : '') + '><a href="javascipt:void(0)">' + p + '</a></li>';
		});
		result +=disptext1+tpage;
...
//		}
	...
	getPagesCount = function () {
		tpage=Math.ceil(itemsCount / pageSize)
		return tpage;
	};
